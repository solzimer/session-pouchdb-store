...
class SessionStore extends Store {
	...
	destroy(sid, callback) {
		this.get(sid,(err,doc)=>{
			if(err) callback(err);
				...
				delete this.sessions[sid];
				this.pouch.remove(doc,callback);
			}
		});
	}
	...
	clear(callback) {
		this.pouch.
			...
			then(deleteDocs => this.pouch.bulkDocs(deleteDocs)).
			then(()=>callback()).
	...
	set(sid, session, callback) {
		...
		sem.take(()=>{
			...
			if(this.sessions[sid]) {
				session._rev = this.sessions[sid]._rev;
			}

			this.pouch.
				put(session).
				then(res=>{
					extend(session,{_rev:res.rev});
					this.sessions[sid] = extend(this.sessions[sid],session);
					sem.leave();
					callback();
				}).
				catch(err=>{
	...
	touch(sid, session, callback) {
		...
		if(oldsession) {
			let rev = oldsession._rev;
			extend(true,oldsession,session,{_rev:rev});
